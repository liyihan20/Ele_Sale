    ...
    public class ApplySv : BaseSv
    {
        ...
        public List<AuditStatusModel> GetAuditStatus()
        {
            ...
            list.Add(new AuditStatusModel()
            {
                ...
                department = new UA((int)ap.user_id).GetUserDepartmentName(),
                step = 0,
                stepName = "提交申请",
                opDate = ((DateTime)ap.start_date).ToString("yyyy-MM-dd HH:mm"),
                pass = true
            });
            ...
            foreach (var ad in ap.ApplyDetails.Where(a => a.pass != null).OrderBy(a => a.finish_date)) {
                list.Add(new AuditStatusModel()
                {
                    ...
                    department = new UA((int)ad.user_id).GetUserDepartmentName(),
                    step = (int)ad.step,
                    stepName = ad.step_name,
                    opDate = ((DateTime)ad.finish_date).ToString("yyyy-MM-dd HH:mm"),
                    pass = ad.pass,
                    comment = ad.comment
                });
            }
            ...
            if (ap.success == true) {
                list.Add(new AuditStatusModel()
                {
                    ...
                    stepName = "申请完成",
                    opDate = ((DateTime)ap.finish_date).ToString("yyyy-MM-dd HH:mm"),
                    pass = true
                });
            }
            return list;
        ...
        private bool IsCurrentStepAuditing(ApplyDetails ad)
        {
            ...
            ap = ad.Apply;
            //if (ap.success != null) return false;  //流程已结束

            //if (ad.step >= 2) {
            //    var ads1 = ap.ApplyDetails.Where(a => a.step == ad.step - 1 && a.pass==true);
            //    if (ads1.Count() == 0) return false; //上一步骤未审核
            //    if (ads1.First().countersign == true) {
            //        if (ap.ApplyDetails.Where(a => a.step == ad.step - 1).Count() != ads1.Count()) {
            //            return false; //上一步骤是会签，且未全部审核
            //        }
            //    }
            //}
