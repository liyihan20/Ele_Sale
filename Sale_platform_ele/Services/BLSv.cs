    ...
    public class BLSv:BillSv
    {
        ...
        private void ExportExcel(List<ExcelData> myData)
        {
            ...
            string[] colName = new string[] { "审核结果","流水号","备料日期","客户型号","版本号","产品类别","客户编码","客户名称","终端客户编码","终端客户名称",
                                            "产品代码","产品名称","产品型号","备料数量(粒)","排版粒数(粒)","订单数量(粒)","营业员","首样时间","量产时间","产品用途",
                                            "FCST周期","制单人","备料项目","备注" };
            ...
            foreach (var d in myData) {
                ...
                cells.Add(rowIndex, ++colIndex, d.h.layout_qty);
                cells.Add(rowIndex, ++colIndex, d.h.order_qty);
                cells.Add(rowIndex, ++colIndex, d.h.clerk_name);
                cells.Add(rowIndex, ++colIndex, d.h.first_date.ToString("yyyy-MM-dd"));
                cells.Add(rowIndex, ++colIndex, d.h.batch_date.ToString("yyyy-MM-dd"));
                cells.Add(rowIndex, ++colIndex, d.h.usage);

                cells.Add(rowIndex, ++colIndex, d.h.fcst);
